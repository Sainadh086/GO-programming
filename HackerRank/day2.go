...
func solve(meal_cost float64, tip_percent int32, tax_percent int32) {
    ...
    tip_cost := (meal_cost/100.0) * float64(tip_percent)
    tax_cost := (meal_cost/100.0) * float64(tax_percent)
    total_cost := meal_cost + tip_cost + tax_cost
    out := total_cost - float64(int64(total_cost))
    if(out >= 0.5){
        fmt.Println(int64(total_cost)+1)
    }else{
    fmt.Println(int64(total_cost))    
    }
